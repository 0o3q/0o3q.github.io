...
Looking at the problem code,
```solidity
if (lastHash == blockValue) {
    revert();
}
```
Since it reverts when `lastHash` and `blockValue` are the same, it's impossible to execute repeatedly with a loop within the same contract.
Therefore, I wrote it to be executed 10 times manually to solve the problem.
